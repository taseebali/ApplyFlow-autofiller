...
- **Dashboard** — every application is saved locally in full: the job description (even from ATS flows where it lives on a different page than the form), the match score and keyword gap, the bank variant ids of the exact bullets that went out, the figures the model estimated, and both the resume and cover letter as the .docx files actually sent. A dashboard at [applyflow-dashboard.vercel.app](https://applyflow-dashboard.vercel.app) renders all of it — it's a static page with no server, no database, and no account, and it holds nothing itself; it asks the extension for records over the browser's own messaging API and shows nothing if the extension isn't installed. Set a status (interview, offer, rejected) from either the panel or the dashboard and it's saved back to the same record.
